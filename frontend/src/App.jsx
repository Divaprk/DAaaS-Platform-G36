...
import { Search, Loader2, DollarSign, Award, Percent, X, ChevronRight, ChevronDown, School, BookOpen, Layers } from 'lucide-react';
...
export default function App() {
  ...
  // NEW: State for the dynamic salary metric
  ...
  // Chart Data with Dynamic Metric Aggregation
        ...
        years.forEach(year => {
          ...
          if(yearData.length > 0) {
            aggregated.push({
              ...
              course_category: cat,
              // DYNAMIC: Calculate the average for whatever metric is currently selected
              [salaryMetric]: yearData.reduce((acc, curr) => acc + (curr[salaryMetric] || 0), 0) / yearData.length,
              employment_rate_overall: yearData.reduce((acc, curr) => acc + (curr.employment_rate_overall || 0), 0) / yearData.length,
              z_score: yearData.reduce((acc, curr) => acc + (curr.z_score || 0), 0) / yearData.length
            });
          }
        });
      });
  ...
  }, [data, viewMode, selectedCourses, selectedCategories, salaryMetric]); // Added salaryMetric to dependencies
  ...
  // DYNAMIC: Selection Stats based on chosen metric
  const selAvgSalary = chartData.length > 0 ? (chartData.reduce((acc, curr) => acc + (curr[salaryMetric] || 0), 0) / chartData.length).toFixed(0) : 0;
  const selAvgEmp = chartData.length > 0 ? (chartData.reduce((acc, curr) => acc + (curr.employment_rate_overall || 0), 0) / chartData.length).toFixed(1) : 0;
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-200 p-8 font-sans">
      ...
      <main className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-1 space-y-4">
          <div className="bg-zinc-900 p-5 rounded-2xl border border-zinc-800 shadow-2xl sticky top-8 max-h-[85vh] flex flex-col">
            ...
            <div className="flex gap-1 mb-4 p-1 bg-zinc-950 rounded-lg border border-zinc-800">
              <button onClick={() => setViewMode('courses')} className={`flex-1 py-1.5 flex justify-center items-center gap-2 text-[9px] font-black uppercase tracking-widest rounded transition-all ${viewMode === 'courses' ? 'bg-zinc-800 text-cyan-400 shadow-lg' : 'text-zinc-600 hover:text-zinc-400'}`}>
                <BookOpen size={10}/> Courses
              </button>
              <button onClick={() => setViewMode('categories')} className={`flex-1 py-1.5 flex justify-center items-center gap-2 text-[9px] font-black uppercase tracking-widest rounded transition-all ${viewMode === 'categories' ? 'bg-zinc-800 text-purple-400 shadow-lg' : 'text-zinc-600 hover:text-zinc-400'}`}>
            ...
            <div className="flex items-center gap-2 mb-4 px-3 py-2 bg-zinc-950 rounded-lg border border-zinc-800">
              ...
              <input className="bg-transparent border-none outline-none text-xs w-full text-white" placeholder="Search..." onChange={(e) => setSearchTerm(e.target.value)} />
            </div>
            ...
            <div className="overflow-y-auto flex-1 pr-2 custom-scrollbar space-y-2">
                          ...
                          <div className="ml-4 flex flex-col gap-1 py-1">
                            {[...groupedData[uni][cat]].filter(c => c.toLowerCase().includes(searchTerm.toLowerCase())).map(course => {
                               ...
                               return (
                                <button
                                  key={course}
                                  onClick={() => selectedCourses.includes(compositeId) ? setSelectedCourses(selectedCourses.filter(c => c !== compositeId)) : setSelectedCourses([...selectedCourses, compositeId])}
              ...
              {viewMode === 'categories' && allCategories.filter(cat => cat.toLowerCase().includes(searchTerm.toLowerCase())).map(cat => (
                <button
                  key={cat}
                  onClick={() => selectedCategories.includes(cat) ? setSelectedCategories(selectedCategories.filter(c => c !== cat)) : setSelectedCategories([...selectedCategories, cat])}
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          <div className="bg-zinc-900/50 p-4 rounded-2xl border border-zinc-800 flex justify-between items-center text-center px-8">
             ...
             <div className="flex gap-8 pl-4 border-l border-zinc-800">
               <div className="space-y-1 text-right">
                 {/* DYNAMIC: Updates based on the metric chosen */}
          ...
          {/* NEW: DYNAMIC METRIC SELECTOR + TOOLS */}
          ...
          <div className="bg-zinc-900/40 border border-zinc-800 rounded-3xl p-8 min-h-[500px]">
             {activeSelections.length > 0 ? (
               <div className="h-[450px] w-full">
                  {activeTool === 'growth' && (
                    <ResponsiveContainer>
                      {/* DYNAMIC: Pass salaryMetric to the formatter */}
                  ...
                  {activeTool === 'tradeoff' && (
                    <ResponsiveContainer>
                      <ScatterChart>
                        ...
                        {/* DYNAMIC: Y-Axis now uses the selected metric */}
                        <YAxis type="number" dataKey={salaryMetric} name="Salary" unit="$" stroke="#71717a" fontSize={10} />
...
// DYNAMIC: Added `metric` argument so the graph plots the user's choice
