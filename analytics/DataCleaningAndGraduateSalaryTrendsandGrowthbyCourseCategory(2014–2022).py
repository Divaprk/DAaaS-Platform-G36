...
ges["course"] = ges["course"].str.replace(
    r"\(\s*\d+\s*[-–]?\s*(yr|year)[s]?\s*(direct\s*)?(honours\s*)?programme\s*\)",
    " ",
    regex=True
)
...
def map_course(course):
    ...
    # 1️⃣ Very specific, protected categories first
    if "medicine" in c:
        return "Medicine"
    ...
    # 4️⃣ IT (must come BEFORE science)
    ...
    # 6️⃣ Accountancy before Business
    if "accountancy" in c:
        return "Accountancy"
...
"""# Analysis 1:
## Graduate Salary Trends and Growth by Course Category (2014–2022)
##Objective
